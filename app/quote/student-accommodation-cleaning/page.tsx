...
export default function StudentAccommodationQuote() {
  ...
  // Accommodation Cleaning Form State
  ...
  const handleCountChange = (field: keyof typeof formData, value: string) => {
    ...
    // Clear step 1 count error if any item is adjusted above 0
    if (errors.counts) {
      setErrors((prev) => {
        const copy = { ...prev };
        delete copy.counts;
  ...
  const validateStep = (currentStep: number) => {
    ...
    if (currentStep === 1) {
      // Validate that at least one room/item has been selected
      const totalItems =
        formData.ensuites +
        formData.nonEnsuites +
        formData.corridors +
        formData.bathrooms +
        formData.studios +
        formData.carpets +
      ...
      if (totalItems === 0) {
        newErrors.counts = "Please select at least one item to clean before proceeding.";
      }
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/quote", {
        ...
        body: JSON.stringify({
          serviceName: "Student Accommodation Cleaning",
          ...formData,
        }),
      });
  ...
  const itemsConfig = [
    { key: "ensuites" as const, label: "Ensuites", icon: Bed },
    { key: "nonEnsuites" as const, label: "Non-Ensuites", icon: Home },
    { key: "corridors" as const, label: "Corridors", icon: DoorOpen },
    { key: "bathrooms" as const, label: "Bathrooms", icon: Bath },
    { key: "studios" as const, label: "Studios", icon: Layers },
    { key: "carpets" as const, label: "Carpets", icon: Wind },
    { key: "fridges" as const, label: "Fridges", icon: Snowflake },
    { key: "ovens" as const, label: "Ovens", icon: Microwave },
  ];
      ...
      <div className="w-full max-w-5xl">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10">
          ...
          >
            ...
            <span>Accommodation Cleaning Specialist</span>
          </motion.div>
          ...
          >
            Book deep cleaning for student houses, university halls, and co-living properties. Select counts below to build your quote.
          </motion.p>
        </div>
              ...
              <div className="bg-secondary/40 rounded-2xl p-4 sm:p-5 mb-8 border border-border/60 relative z-10">
                <div className="flex justify-between items-center mb-4">
                  <span className="font-extrabold text-sm sm:text-base tracking-wide text-foreground uppercase">
                    Step {step}: {step === 1 ? "Accommodation Details" : "Your Details"}
                  </span>
                ...
                <div className="relative flex items-center justify-between px-6 mt-6 pb-8">
                  ...
                  <div className="relative z-10 flex flex-col items-center">
                    ...
                    <span className="hidden sm:block text-[10px] sm:text-xs font-bold text-foreground absolute top-12 whitespace-nowrap">Accommodation Details</span>
                  </div>
                  ...
                  {step === 1 ? (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Select the number of units requiring sanitization and deep cleaning.</p>
                      </div>
            ...
            >
              <div className="bg-card text-card-foreground rounded-3xl p-8 sm:p-12 border border-border shadow-lg relative overflow-hidden flex flex-col items-center">
                ...
                <p className="text-muted-foreground max-w-md text-sm sm:text-base leading-relaxed mb-8 relative z-10">
                  Thank you, <span className="font-bold text-foreground">{formData.firstName}</span>. Your request for Student & Young Professionals Accommodation Cleaning has been received. Our team will review the details and reach out with a custom quote within 24 hours.
                </p>
